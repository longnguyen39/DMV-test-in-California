...
class ViewControllerTest1: UIViewController {
    ...
    @IBAction func checkButton(_ sender: AnyObject) {
        ...
        disableButton()
        explanationLabel.text = "Answer:  \(AnswerExplanation1().list[questionIndexNumber])"
    ...
    func UIupdate () {
        if questionIndexNumber < QuestionBank1().list.count {
        scoreLabel.text = "\(questionIndexNumber+1) / \(QuestionBank1().list.count)"
    ...
    func checkfunc () {
        if questionIndexNumber < QuestionBank1().list.count {
            ...
            if pickedAnswer == QuestionBank1().list[questionIndexNumber].answer {
            print ("Yeah!!!")
            score = score + 1
            colorTrue()
    ...
    func colorCorrect () {
        if QuestionBank1().list[questionIndexNumber].answer == "A" {
            ALabel.backgroundColor = UIColor.green
        }
        else if QuestionBank1().list[questionIndexNumber].answer == "B" {
            BLabel.backgroundColor = UIColor.green
        }
        else if QuestionBank1().list[questionIndexNumber].answer == "C" {
            CLabel.backgroundColor = UIColor.green
        }
        else if QuestionBank1().list[questionIndexNumber].answer == "D" {
            DLabel.backgroundColor = UIColor.green
        }
    }
    ...
    func nextQuestion () {
        questionIndexNumber = questionIndexNumber + 1
        if questionIndexNumber < QuestionBank1().list.count {
            questionLabel.text = QuestionBank1().list[questionIndexNumber].questionText
            
            ALabel.text = " A. \(QuestionBank1().list[questionIndexNumber].A)"
            BLabel.text = " B. \(QuestionBank1().list[questionIndexNumber].B)"
            CLabel.text = " C. \(QuestionBank1().list[questionIndexNumber].C)"
            DLabel.text = " D. \(QuestionBank1().list[questionIndexNumber].D)"
                
            UIupdate()
        }
            else {
                print("done")
            finalScore = (Double(score)/Double(QuestionBank1().list.count))*100
