...
class ViewControllerTest4: UIViewController {
    ...
    @IBAction func checkButton(_ sender: AnyObject) {
        ...
        disableButton()
        explanationLabel.text = "Answer:  \(AnswerExplanation4().list[questionIndexNumber])"
    ...
    func UIupdate () {
        if questionIndexNumber < QuestionBank4().list.count {
        scoreLabel.text = "\(questionIndexNumber+1) / \(QuestionBank4().list.count)"
    ...
    func checkfunc () {
        if questionIndexNumber < QuestionBank4().list.count {
            ...
            if pickedAnswer == QuestionBank4().list[questionIndexNumber].answer {
            print ("Yeah!!!")
            score = score + 1
            colorTrue()
        ...
        func colorCorrect () {
            if QuestionBank4().list[questionIndexNumber].answer == "A" {
                ALabel.backgroundColor = UIColor.green
            }
            else if QuestionBank4().list[questionIndexNumber].answer == "B" {
                BLabel.backgroundColor = UIColor.green
            }
            else if QuestionBank4().list[questionIndexNumber].answer == "C" {
                CLabel.backgroundColor = UIColor.green
            }
            else if QuestionBank4().list[questionIndexNumber].answer == "D" {
                DLabel.backgroundColor = UIColor.green
            }
        }
    ...
    func nextQuestion () {
        questionIndexNumber = questionIndexNumber + 1
        if questionIndexNumber < QuestionBank4().list.count {
            questionLabel.text = QuestionBank4().list[questionIndexNumber].questionText
            
            ALabel.text = " A. \(QuestionBank4().list[questionIndexNumber].A)"
            BLabel.text = " B. \(QuestionBank4().list[questionIndexNumber].B)"
            CLabel.text = " C. \(QuestionBank4().list[questionIndexNumber].C)"
            DLabel.text = " D. \(QuestionBank4().list[questionIndexNumber].D)"
                
            UIupdate()
        }
            else {
                print("done")
            finalScore = (Double(score)/Double(QuestionBank4().list.count))*100
            let final: String = String(format: "%0.1f", finalScore)
            
                let alert = UIAlertController(title: "You are done!! Your score is: \(final)%, with \(score) corrected out of \(QuestionBank4().list.count)", message: "Do you want to start again?", preferredStyle: .alert)
